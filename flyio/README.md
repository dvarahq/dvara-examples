...
export DVARA_LICENSE_KEY=DVARA-…                                    # production-signed envelope
export DVARA_ENCRYPTION_MASTER_PASSWORD=$(openssl rand -base64 32)  # ESCROW OFFLINE
...
fly secrets set \
  DVARA_LICENSE_KEY="$DVARA_LICENSE_KEY" \
  DVARA_ENCRYPTION_MASTER_PASSWORD="$DVARA_ENCRYPTION_MASTER_PASSWORD" \
  DVARA_ACTUATOR_API_KEY="$DVARA_ACTUATOR_API_KEY" \
  DVARA_ACTUATOR_METRICS_API_KEY="$DVARA_ACTUATOR_METRICS_API_KEY"
...
# Authenticated:
curl -H "Authorization: Bearer $DVARA_ACTUATOR_API_KEY" \
...
- **No flightdeck Console** — single-service template. Add a second app for `flightdeck/Dockerfile`, attach the same Postgres, set `DVARA_FLIGHTDECK_GATEWAY_URL` to the gateway's `<app>.flycast` internal address.
