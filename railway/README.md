...
|---|---|
| `DVARA_LICENSE_KEY` | Production-signed `DVARA-…` envelope from your DVARA account team |
| `DVARA_ENCRYPTION_MASTER_PASSWORD` | `openssl rand -base64 32` — **escrow offline** (password manager + printed copy in a safe) |
| `DVARA_ACTUATOR_API_KEY` | `openssl rand -base64 32` |
| `DVARA_ACTUATOR_METRICS_API_KEY` | `openssl rand -base64 32` — **must differ** from `DVARA_ACTUATOR_API_KEY` (principle of least privilege) |
...
- **No `flightdeck` Console** — the template deploys only `gateway-server`. To get the Console + tenant Portal, deploy a second service from `flightdeck/Dockerfile` and wire `DVARA_FLIGHTDECK_GATEWAY_URL` to the gateway-server's internal Railway URL.
