...
Both variants default to `:latest`. For production stability, replace every `image.tag: latest` with the published tag — e.g. `1.7.0` after the GA tag fires. Check the available tags at the [GHCR package page](https://github.com/orgs/dvarahq/packages/container/package/dvara%2Fdvara-gateway).
