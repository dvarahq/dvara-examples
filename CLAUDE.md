...
- **Images are pinned to an explicit version tag (`:1.6.0`)** in every stack — not `:latest` — so a copied stack is reproducible. Bump the pin in each `docker-compose.yml` when a new release ships.
