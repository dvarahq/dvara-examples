...
- `curl http://localhost:8080/mcp/` — the MCP plane, on the gateway (`404` unlicensed). Port 8070 is RETIRED
...
- **All Dvara services share one database.** `dvara-gateway` and `dvara-flightdeck` both connect to the same `dvara` DB with the same credentials. Do not give them separate databases.
...
- **Service dependency chain is fixed**: postgres (healthy) → dvara-gateway (healthy) → dvara-flightdeck. Preserve these `depends_on` + `condition: service_healthy` blocks when editing.
...
- **Images are pinned to an explicit version tag (`:1.7.0`)** in every stack — not `:latest` — so a copied stack is reproducible. Bump the pin in each `docker-compose.yml` when a new release ships.
