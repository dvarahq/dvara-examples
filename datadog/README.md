...
2. **Set the Bearer auth secret.** On DVARA rc24+, `/actuator/prometheus` is locked down behind `GATEWAY_METRICS_API_KEY` (per `dvarahq/dvara#665`). The shipped config uses Datadog Agent's secret backend syntax:
   ...
   ```yaml
   headers:
     Authorization: "Bearer <paste GATEWAY_METRICS_API_KEY here>"
   ```
