...
`deploy.sh` and `values-gke.yaml` default to a published GA tag (`1.2.5`). Never run `:latest` in production. Browse tags at the [GHCR package page](https://github.com/orgs/dvarahq/packages?repo_name=dvara); set `IMAGE_TAG` / `CHART_VERSION` to match.
