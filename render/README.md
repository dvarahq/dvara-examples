...
| `DVARA_ENCRYPTION_MASTER_PASSWORD` | `openssl rand -base64 32` — **escrow offline** |
| `DVARA_ACTUATOR_API_KEY` | `openssl rand -base64 32` |
| `DVARA_ACTUATOR_METRICS_API_KEY` | `openssl rand -base64 32` — must differ from `DVARA_ACTUATOR_API_KEY` |
...
- **No flightdeck Console** — single-service template. Add a second `web` service from `flightdeck/Dockerfile` and wire `DVARA_FLIGHTDECK_GATEWAY_URL` to the gateway service's internal URL.
